...
export function TraderDashboardPage({
  ...
  const [showBulkCleanupMenu, setShowBulkCleanupMenu] = useState<boolean>(false)

  // Clean error records API call
  const cleanErrorRecords = async (traderId: string | undefined) => {
    if (!traderId) {
      alert('Please select a trader first');
      return;
    }
    if (!confirm('Are you sure you want to clean all error records for this trader?')) {
      return;
    }

    try {
      const response = await fetch(`/api/v1/decisions/clean-errors?trader_id=${encodeURIComponent(traderId)}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      ...
      alert(`Cleaned ${result.deleted_count} error records!`);
      window.location.reload(); // Refresh the page to see the changes
    } catch (error) {
      console.error('Error cleaning error records:', error);
      alert('Failed to clean error records: ' + (error as Error).message);
    }
  };

  // Clean invalid records API call (pure errors without calculation results)
  const cleanInvalidRecords = async (traderId: string | undefined) => {
    if (!traderId) {
      alert('Please select a trader first');
      return;
    }
    if (!confirm('Are you sure you want to clean all invalid records (pure errors without calculation results) for this trader?')) {
      return;
    }

    try {
      const response = await fetch(`/api/v1/decisions/clean-invalid?trader_id=${encodeURIComponent(traderId)}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      ...
      const result = await response.json();
      alert(`Cleaned ${result.deleted_count} invalid records!`);
      window.location.reload(); // Refresh the page to see the changes
    } catch (error) {
      console.error('Error cleaning invalid records:', error);
      alert('Failed to clean invalid records: ' + (error as Error).message);
    }
  };

  // Clean records before time API call
  const cleanBeforeTime = async (traderId: string | undefined, days: number) => {
    if (!traderId) {
      alert('Please select a trader first');
      return;
    }
    if (!confirm(`Are you sure you want to clean all records older than ${days} days for this trader?`)) {
      return;
    }

    const beforeTime = new Date();
    beforeTime.setDate(beforeTime.getDate() - days);
    const timeString = beforeTime.toISOString();

    try {
      const response = await fetch(`/api/v1/decisions/clean-before-time?trader_id=${encodeURIComponent(traderId)}&before_time=${encodeURIComponent(timeString)}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      ...
      const result = await response.json();
      alert(`Cleaned ${result.deleted_count} records older than ${days} days!`);
      window.location.reload(); // Refresh the page to see the changes
    } catch (error) {
      console.error('Error cleaning records before time:', error);
      alert('Failed to clean records: ' + (error as Error).message);
    }
  };
          ...
          >
            ...
            <div className="flex items-center gap-2 mb-3">
              <div className="flex gap-2 flex-wrap">
                ...
                <div className="relative">
                  <button
                    onClick={() => setShowBulkCleanupMenu(!showBulkCleanupMenu)}
                  ...
                  {showBulkCleanupMenu && (
                    <div
                      className="absolute right-0 mt-2 w-64 rounded-lg shadow-lg py-2 z-10"
                      style={{
                        background: '#1E2329',
                      ...
                      onMouseLeave={() => setShowBulkCleanupMenu(false)}
                    >
                      <div className="py-1">
                        <button
                          onClick={(e) => {
                            e.preventDefault();
                            if (selectedTraderId) {
                              cleanErrorRecords(selectedTraderId);
                            } else {
                              alert('Please select a trader first');
                            }
                          }}
                          className="block w-full text-left px-4 py-2 text-sm hover:bg-red-500/10 transition-colors"
                          style={{ color: '#F6465D' }}
                        >
                          🗑️ Clean Error Records
                        </button>
                        ...
                        <button
                          onClick={(e) => {
                            e.preventDefault();
                            if (selectedTraderId) {
                              cleanInvalidRecords(selectedTraderId);
                            } else {
                              alert('Please select a trader first');
                            }
                          }}
                        ...
                        <div className="border-t my-1" style={{ borderColor: '#2B3139' }} />
                        
                        <button
                          onClick={(e) => {
                            e.preventDefault();
                            if (selectedTraderId) {
                              cleanBeforeTime(selectedTraderId, 7);
                            } else {
                              alert('Please select a trader first');
                            }
                          }}
                          className="block w-full text-left px-4 py-2 text-sm hover:bg-yellow-500/10 transition-colors"
                          style={{ color: '#F0B90B' }}
                        >
                          📅 Clean 7+ Days Old
                        </button>
                        ...
                        <button
                          onClick={(e) => {
                            e.preventDefault();
                            if (selectedTraderId) {
                              cleanBeforeTime(selectedTraderId, 30);
                            } else {
                              alert('Please select a trader first');
                            }
                          }}
                        ...
                        <button
                          onClick={(e) => {
                            e.preventDefault();
                            if (selectedTraderId) {
                              cleanBeforeTime(selectedTraderId, 90);
                            } else {
                              alert('Please select a trader first');
                            }
                          }}
