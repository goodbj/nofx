...
function App() {
  ...
  const { data: exchanges, mutate: mutateExchanges } = useSWR<Exchange[]>(
  ...
  // 优化交易所数据加载 - 确保认证完成后延迟加载
  useEffect(() => {
    if (user && token) {
      // 延迟1秒加载交易所数据，确保认证状态稳定
      const timer = setTimeout(() => {
        mutateExchanges()
      }, 1000)
      return () => clearTimeout(timer)
    }
  }, [user, token, mutateExchanges])
            ...
            <div className="mt-4 flex items-center justify-center gap-3 flex-wrap">
              ...
              >
                <svg
                  ...
                  fill="currentColor"
                >
                  <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27.53.25.94.81 1.15 1.46.21.65.3.14.76.55.46.41.49 1.04.52 1.64.03.6.03 1.19.03 1.79 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z" />
                </svg>
                GitHub
              </a>
              {/* Twitter/X */}
