...
[
  {
    ...
    "confidence": 85,
    "risk_usd": 300,
    "reasoning": "BTC价格跌破95000支撑位，MACD死叉确认下跌趋势，持仓量减少8%表明多头平仓，RSI超卖但动能强劲，风险回报比1:3可接受，执行做空。"
  }
]
```
</decision>
...
JSON数组中的每个决策对象**必须**包含以下字段：

**必填字段**：
- `symbol`: 交易对（如 "BTCUSDT"）
- `action`: 动作类型（open_long, open_short, close_long, close_short, hold, wait等）
- `confidence`: 置信度 0-100
- `reasoning`: **详细的决策逻辑说明**（必需）

**条件字段**（根据动作类型需要）：
- `leverage`: 开新仓时必需
- `position_size_usd`: 开新仓时必需
- `stop_loss`: 开新仓时建议提供
- `take_profit`: 开新仓时建议提供

#### reasoning字段指南

`reasoning`字段对于理解AI决策逻辑至关重要：

**目的**：解释分析逻辑和决策依据，而不仅仅是具体操作
...
**✅ 好的例子**：
```
"技术指标共振，多时间框架趋势一致，持仓量确认多头动能，风险回报比可接受"

"当前持仓从峰值回撤5%，趋势反转信号出现，是时候保护资本了"
```

**❌ 应避免的差例子**：
```
"执行3倍杠杆的多头仓位"  # 描述动作，而非推理

"设置止损42000和止盈48000"  # 执行细节
