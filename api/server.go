...
func (s *Server) handleCreateTrader(c *gin.Context) {
	...
	if err != nil {
		logger.Infof("â?Failed to create trader: %v", err)
		...
		if reloadedTrader, getErr := s.traderManager.GetTrader(traderID); getErr == nil {
			go func() {
				...
				if runErr := reloadedTrader.Run(); runErr != nil {
					logger.Infof("â?Trader %s runtime error: %v", traderID, runErr)
				}
			}()
		}
	}
...
func (s *Server) handleStartTrader(c *gin.Context) {
	...
	if loadErr := s.traderManager.LoadUserTradersFromStore(s.store, userID); loadErr != nil {
		logger.Infof("â?Failed to load user traders: %v", loadErr)
	...
	go func() {
		...
		if err := trader.Run(); err != nil {
			logger.Infof("â?Trader %s runtime error: %v", trader.GetName(), err)
		}
	}()
...
func (s *Server) handleExecuteDecision(c *gin.Context) {
	...
	if err != nil {
		logger.Errorf("â?Failed to trigger decision for trader %s: %v", trader.GetName(), err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to trigger decision"})
		return
	}

	logger.Infof("âœ?Manual decision executed successfully for trader %s: %+v", trader.GetName(), result)
	c.JSON(http.StatusOK, gin.H{
...
func (s *Server) handleSyncBalance(c *gin.Context) {
	...
	if err != nil {
		logger.Infof("â?Failed to update initial_balance: %v", err)
...
func (s *Server) handleClosePosition(c *gin.Context) {
	...
	if closeErr != nil {
		logger.Infof("â?Close position failed: symbol=%s, side=%s, error=%v", req.Symbol, req.Side, closeErr)
...
func (s *Server) handleGetModelConfigs(c *gin.Context) {
	...
	if err != nil {
		logger.Infof("â?Failed to get AI model configs: %v", err)
...
func (s *Server) handleUpdateModelConfigs(c *gin.Context) {
	...
	if !cfg.TransportEncryption {
		...
		if err := json.Unmarshal(bodyBytes, &req); err != nil {
			logger.Infof("â?Failed to parse plain JSON request: %v", err)
		...
		if err := json.Unmarshal(bodyBytes, &encryptedPayload); err != nil {
			logger.Infof("â?Failed to parse encrypted payload: %v", err)
		...
		if encryptedPayload.WrappedKey == "" {
			logger.Infof("â?Detected unencrypted request (UserID: %s)", userID)
		...
		if err != nil {
			logger.Infof("â?Failed to decrypt model config (UserID: %s): %v", userID, err)
		...
		if err := json.Unmarshal([]byte(decrypted), &req); err != nil {
			logger.Infof("â?Failed to parse decrypted data: %v", err)
...
func (s *Server) handleUpdateExchangeConfigs(c *gin.Context) {
	...
	if !cfg.TransportEncryption {
		...
		decrypted, err := s.cryptoHandler.cryptoService.DecryptSensitiveData(&encryptedPayload)
		if err != nil {
			logger.Infof("â?Failed to decrypt exchange config (UserID: %s): %v", userID, err)
			c.JSON(http.StatusBadRequest, gin.H{"error": "Failed to decrypt data"})
			return
		}
...
func (s *Server) handleCreateExchange(c *gin.Context) {
	...
	if err != nil {
		logger.Infof("â?Failed to create exchange account: %v", err)
...
func (s *Server) handleDeleteExchange(c *gin.Context) {
	...
	if err != nil {
		logger.Infof("â?Failed to delete exchange account: %v", err)
...
func (s *Server) getKlinesFromCoinank(symbol, interval, exchange string, limit int) ([]market.Kline, error) {
	...
	// Coinank: Volume = BTC æ•°é‡, Quantity = USDT æˆäº¤é¢?
	...
	for i, ck := range coinankKlines {
		klines[i] = market.Kline{
			...
			QuoteVolume: ck.Quantity, // USDT æˆäº¤é¢?
...
func (s *Server) getKlinesFromAlpaca(symbol, interval string, limit int) ([]market.Kline, error) {
	...
	for i, bar := range bars {
		klines[i] = market.Kline{
			...
			QuoteVolume: float64(bar.Volume) * bar.Close, // æˆäº¤é¢?= è‚¡æ•° * æ”¶ç›˜ä»?(USD)
...
func (s *Server) getKlinesFromHyperliquid(symbol, interval string, limit int) ([]market.Kline, error) {
	...
	for i, candle := range candles {
		...
		klines[i] = market.Kline{
			...
			Low:         low,
			Close:       close,
			Volume:      volume,         // åˆçº¦æ•°é‡
			QuoteVolume: volume * close, // æˆäº¤é¢?(USD)
