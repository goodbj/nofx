...
func demoCoreLogic() {
	...
	for _, tc := range testCases {
		result := manager.alignToStepSize(tc.quantity, tc.stepSize)
		formatted := manager.formatWithPrecision(result, calculatePrecision(tc.stepSize))
		...
		fmt.Printf("  %s %s: %.12f (stepSize: %f) -> %s (期望: %s)\n",
			status, tc.desc, tc.quantity, tc.stepSize, formatted, tc.expected)
	}
}

// MockPrecisionManager 模拟精度管理器的核心逻辑
type MockPrecisionManager struct{}

func (m *MockPrecisionManager) alignToStepSize(quantity, stepSize float64) float64 {
	if stepSize <= 0 {
		return quantity
	}
...
func (m *MockPrecisionManager) formatWithPrecision(quantity float64, precision int) string {
	if precision < 0 {
		precision = 3
	}
