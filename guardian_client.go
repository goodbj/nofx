...
func (gc *GuardianClient) performBrowserAutomation(prompt string) (string, error) {
		...
		chromedp.ActionFunc(func(ctx context.Context) error {
			...
			for _, selector := range inputSelectors {
				...
				if err == nil {
					...
					// 清空输入框并输入新内容
					gc.logger.Printf("??? Clearing input field: %s", selector)
					err = chromedp.Clear(selector).Do(ctx)
					if err != nil {
						gc.logger.Printf("?? Could not clear input field %s, proceeding anyway: %v", selector, err)
					}
					...
					if err != nil {
						...
						// 直接跳过此选择器
						gc.logger.Printf("?? Skipping this selector due to focus activation failure")
						continue // 继续尝试下一个选择器
					} else {
						...
						err = chromedp.Clear(selector).Do(ctx)
						if err != nil {
							gc.logger.Printf("?? Could not clear input field, proceeding anyway: %v", err)
						}
						...
						err = chromedp.Focus(selector).Do(ctx)
						if err != nil {
							gc.logger.Printf("?? Focus failed: %v", err)
						}
						...
						if err != nil {
							...
							// 直接跳过此选择器
							gc.logger.Printf("?? Skipping this selector due to SetValue failure")
							continue // 继续尝试下一个选择器
						}
						...
						if err != nil {
							gc.logger.Printf("?? SendKeys activation character failed: %v", err)
						...
						if err != nil || !backspaceResult {
							gc.logger.Printf("?? JavaScript Backspace simulation failed: %v, success: %v", err, backspaceResult)
						} else {
						...
						if err != nil || !ensureResult {
							gc.logger.Printf("?? State update script failed: %v, success: %v", err, ensureResult)
						} else {
	...
	waitErr := chromedp.Run(ctx,
		chromedp.ActionFunc(func(ctx context.Context) error {
			...
			for time.Now().Before(maxWait) {
						...
						if inputLength == len(safePrompt) {
							...
							for _, submitSelector := range submitSelectors {
								...
								if err == nil {
									...
									gc.logger.Printf("?? Failed to click submit button %s: %v, trying next selector", submitSelector, err)
								}
							}
							// 如果所有提交按钮都点击失败，继续等待
						}
				...
				if !foundInput {
					gc.logger.Println("?? Could not find input box, continuing to wait...")
				}
	...
	if waitErr != nil {
		gc.logger.Printf("?? Wait for input completion failed or timeout: %v", waitErr)
		...
		if clickErr != nil {
			gc.logger.Printf("?? All submit button attempts failed: %v", clickErr)
		}
	}
	...
	if err != nil {
		gc.logger.Printf("?? Error waiting for AI completion: %v", err)
		// 继续执行，即使等待出现问题
	} else {
	...
	if err != nil {
		gc.logger.Printf("?? Error getting page HTML: %v", err)
	}
	...
	if err != nil {
		gc.logger.Printf("?? Error getting ds-theme elements detail: %v", err)
	}
	...
	if err != nil {
		gc.logger.Printf("?? Error getting potential AI completion elements: %v", err)
	}
	...
	if err != nil {
		gc.logger.Printf("?? Error getting all page elements: %v", err)
	}
	...
	if err != nil {
		gc.logger.Printf("?? Error getting DOM tree: %v", err)
	}

	// 检查是否已经检测到AI输出完成，如果是，则跳过主等待循环
	if extractedContent != "" {
		gc.logger.Println("?? Skipping main wait loop as AI output has already been detected and captured")
		// 直接跳到函数末尾，避免进入主等待循环
		// 不再等待主循环，直接执行清理并返回
...
Loop:
						...
						default:
							...
							for _, submitSel := range submitSelectors {
								...
								if err == nil && buttonChanged {
									gc.logger.Println("???? WARNING: 已经检测到输出完成，准备复制 ????")
									break Loop
								}
							}
							...
							for _, copySelector := range copySelectors {
								...
								if err == nil && copyButtonExists {
									...
									if clickErr == nil {
										...
										gc.logger.Printf("?? Failed to click first copy button %s: %v, trying original selector", firstCopySelector, clickErr)
										// 如果第一个选择器失败，尝试原来的选择器
										...
										if clickErr2 == nil {
											...
											gc.logger.Printf("?? Failed to click copy button %s: %v", copySelector, clickErr2)
										}
									}
									...
									for _, copySelector := range copySelectors {
										...
										if err == nil {
											...
											if clickErr == nil {
												...
												gc.logger.Printf("?? Failed to click copy button %s: %v", copySelector, clickErr)
											}
										} else {
											gc.logger.Printf("?? Copy button %s not visible: %v", copySelector, err)
										}
									}

									if !clicked {
										gc.logger.Println("?? Failed to click any copy button after detecting completion text")
									}
	...
	gc.logger.Printf("?? Browser automation completed in %v", duration)
...
func (gc *GuardianClient) CheckLoginStatus(targetURL string) (bool, error) {
	...
	if err != nil {
		// 如果评估失败，假设未登录
		gc.logger.Printf("?? Error checking login status: %v, assuming not logged in", err)
		return false, nil
	}
		...
		default:
			...
			if err != nil {
				...
				if containsAICompletion {
					...
					if extractErr != nil {
						gc.logger.Printf("?? Error extracting AI output content: %v", extractErr)
						// 如果提取失败，仍然返回成功，但内容为空
						return "", nil
					}
...
func (gc *GuardianClient) ProcessAIOutputCompletion(ctx context.Context) error {
	...
	if err != nil {
		gc.logger.Printf("?? 获取页面HTML失败: %v", err)
		return err
	}
