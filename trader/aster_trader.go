...
func (t *AsterTrader) PartialClose(symbol string, side string, percentage float64) (map[string]interface{}, error) {
...
func (t *AsterTrader) UpdateStopLoss(symbol string, positionSide string, newStopPrice float64) error {
	// 先取消现有的止损订单
	...
	logger.Infof("✓ [Aster] Stop loss updated for %s to %.4f", symbol, newStopPrice)
	return nil
}

// UpdateTakeProfit 更新止盈单
func (t *AsterTrader) UpdateTakeProfit(symbol string, positionSide string, newTakeProfitPrice float64) error {
	// 先取消现有的止盈订单
	...
	logger.Infof("✓ [Aster] Take profit updated for %s to %.4f", symbol, newTakeProfitPrice)
	return nil
}
