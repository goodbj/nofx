...
echo.
echo 正在查找并终止 Guardian 进程...
taskkill /f /im main.exe 2>nul
if %errorlevel% equ 0 (echo 已终止 Guardian 进程) else (echo 未找到 Guardian 进程)

echo.
echo 正在查找并终止 Chrome 进程 (Guardian浏览器自动化)...
taskkill /f /im chrome.exe 2>nul
if %errorlevel% equ 0 (echo 已终止 Chrome 进程) else (echo 未找到 Chrome 进程)

echo.
echo 正在查找并终止 chromedriver 进程...
taskkill /f /im chromedriver.exe 2>nul
if %errorlevel% equ 0 (echo 已终止 chromedriver 进程) else (echo 未找到 chromedriver 进程)
