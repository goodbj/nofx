...
class NOFXLauncherAdvanced:
    def __init__(self, root):
        self.root = root
        self.root.title("NOFX 开发版启动器 - 高级版 v1.0")
        ...
        # 过滤标签
        tk.Label(filter_frame, text="日志过滤:", font=("Arial", 9)).pack(side=tk.LEFT, padx=(0, 5))
        ...
        # 过滤按钮
        self.filter_btn = tk.Button(filter_btn_frame, text="筛选", command=self.apply_filter, bg="#95a5a6", fg="white", font=("Arial", 8))
        self.filter_btn.pack(side=tk.LEFT, padx=(0, 2))
        
        # 清除过滤按钮
        self.clear_filter_btn = tk.Button(filter_btn_frame, text="清除", command=self.clear_filter, bg="#e74c3c", fg="white", font=("Arial", 8))
        self.clear_filter_btn.pack(side=tk.LEFT)
        
        # 预设过滤按钮
        preset_frame = tk.Frame(filter_frame)
        preset_frame.pack(side=tk.LEFT, padx=(0, 10))
        
        tk.Button(preset_frame, text="后端", command=lambda: self.set_filter("BACKEND"), bg="#3498db", fg="white", font=("Arial", 8)).pack(side=tk.LEFT, padx=(0, 2))
        tk.Button(preset_frame, text="前端", command=lambda: self.set_filter("FRONTEND"), bg="#3498db", fg="white", font=("Arial", 8)).pack(side=tk.LEFT, padx=(0, 2))
        tk.Button(preset_frame, text="错误", command=lambda: self.set_filter("ERROR"), bg="#e74c3c", fg="white", font=("Arial", 8)).pack(side=tk.LEFT, padx=(0, 2))
        tk.Button(preset_frame, text="警告", command=lambda: self.set_filter("WARN"), bg="#f39c12", fg="white", font=("Arial", 8)).pack(side=tk.LEFT, padx=(0, 2))
        ...
        # 添加日志标题
        self.log_message("=== NOFX 开发版启动器 - 高级版 ===\n")
        ...
        self.log_message("- 使用上方过滤框筛选日志内容\n")
        self.log_message("- 点击预设过滤按钮快速筛选特定类型日志\n")
        self.log_message("----------------------------------------\n")
    ...
    def show_about(self):
        """显示关于对话框"""
        messagebox.showinfo("关于", "NOFX 开发版启动器 - 高级版 v1.0\n\n功能增强版启动器，包含日志过滤功能\n\n开发版后端端口: 8888\n开发版前端端口: 3300")

def main():
    root = tk.Tk()
    app = NOFXLauncherAdvanced(root)
